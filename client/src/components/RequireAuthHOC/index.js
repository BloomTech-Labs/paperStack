    ...
    componentWillMount() {
      ...
      this.setState({ verifyingToken: true }, () => {
        axios
          ...
          .catch(err => {
            this.setState(
              { authenticated: false, verifyingToken: false },
              () => {
                setTimeout(() => {
                  this.props.history.push("/authorisation");
                }, 5000);
      ...
      if (!this.state.authenticated && !this.state.verifyingToken) {
        return (
          <div className="Require-Auth">
            <p>
              You are not authorized to access this web page. <br />Please
              verify you are authorized and re-enter your User ID and Password.
            </p>
          </div>
        );
