...
class SignUpForm extends Component {
  ...
  handleSubmit = (event) => {
    ...
    if (!confirmPassword) { return this.setState({ confirmPasswordErr: '*this field is required' }); }
    ...
    if (password.length < 8) return this.setState({ passwordErr: '*must be at least 8 characters' });
    else this.setState({ passwordErr: '' });
    ...
    if (password !== confirmPassword) return this.setState({ confirmPasswordErr: '*passwords do not match' });
    else this.setState({ confirmPasswordErr: '' });
    ...
    return (
      <form>
        ...
        <input type="text" className="Auth-Form-field" spellCheck="false"
          value={this.state.username} onChange={this.handleEmailChange} />
